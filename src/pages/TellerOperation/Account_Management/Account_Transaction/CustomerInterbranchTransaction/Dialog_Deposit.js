...
// ----- MAIN -----
export default function Dialog_Deposit({CustomerID , object}) {
    // Fetch Data 
    // Show notification
  ...
  return (
    <div>
      ...
      >
        <AppBar sx={{ position: 'relative' , backgroundColor: '#d71921'}}>
          <Toolbar>
            ...
            <Typography sx={{ ml: 2, flex: 1 }} variant="h6" component="div">
                Cash Deposits - Ref ID: {CustomerID}
            </Typography>
            <Button startIcon={<CheckCircleOutlineIcon />} autoFocus color="inherit" onClick={ async () =>{

              let params = {}
              params.Status = resolveNameID(StatusArray,document.getElementById('slt_Status_CashDeposits_Popup').innerText);
              const res = await cashDepositsApi.postValidate(CustomerID, params);
              if(res != 'fail') {
        ...
        {isDisabledDialog && <CashDepositsComponents suffixID='CashDeposits_Popup' forceDisable={true} object={object}/>}
        {!isDisabledDialog && <CashDepositsComponents suffixID='CashDeposits_Popup' object={object}/>}

        {isNotification_Success_01 && <Message_String type='success' text='Update Validation of Cash Deposits Successfully'/>}                  
        {isNotification_Failed_01 && <Message_String type='error' text='Update Validation of Cash Deposits Failed'/>}  
