...
// ----- MAIN -----
export default function Dialog_Withdrawal({CustomerID , object}) {
    // Fetch Data 
    // Show notification
  ...
  return (
    <div>
      ...
      >
        <AppBar sx={{ position: 'relative' , backgroundColor: '#d71921'}}>
          <Toolbar>
            ...
            <Typography sx={{ ml: 2, flex: 1 }} variant="h6" component="div">
                Cash Withdrawal - Ref ID: {CustomerID}
            </Typography>
            <Button startIcon={<CheckCircleOutlineIcon />} autoFocus color="inherit" onClick={ async () =>{

              let params = {}
              params.Status = resolveNameID(StatusArray,document.getElementById('slt_Status_CashWithdrawal_Popup').innerText);
              const res = await cashDepositsApi.postValidateWithdrawal(CustomerID, params);
              if(res != 'fail') {
        ...
        {isDisabledDialog && <CashWithdrawalComponents suffixID='CashWithdrawal_Popup' forceDisable={true} object={object}/>}
        {!isDisabledDialog && <CashWithdrawalComponents suffixID='CashWithdrawal_Popup' object={object}/>}
