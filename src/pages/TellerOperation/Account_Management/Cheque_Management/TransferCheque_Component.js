...
function TransferCheque_Component({suffixID, forceDisable, object}) {
    ...
    object = ""
        ...
        subCustomer = object.DebitAccountt
        isValidate = true
        isPopup = true
    }
      return ( 
        ...
        >
          <Box m={2}>
                            ...
                            if(chequeIDEle){
                                chequeDB.map(value => {
                                    if(chequeIDEle == value.ChequeID){
                                        ...
                                        let amountLCY = document.getElementById('txt_DebitAmount_'+suffixID).value 
                                        document.getElementById('txt_ChequeNoTemp_'+suffixID).value= `${value.ChequeNoStart} - ${value.ChequeNoEnd}`
                                        if(parseInt(amountLCY)){
                                            let paidAmount = parseInt(amountLCY) * parseInt(document.getElementById('txt_DealRate_'+suffixID).value)
                                            document.getElementById('txt_AmountPaid_'+suffixID).value = paidAmount
                                            document.getElementById('txt_NewAmount_'+suffixID).value=value.DEBITACCOUNT.WorkingAmount - paidAmount
                                        }
                                    }
            ...
            >
                ...
                <Block_Children header2='DEBIT INFOMATION'>
                    <TextField_Value id={'txt_Currency_'+suffixID} label='Currency' length='25' disabled={true} value={object.DebitCurrencyt?.Name? object.DebitCurrencyt.Name : ''} noDown={true}/>
                    ...
                    <TextField_Value id={'txt_DebitAmount_'+suffixID} label='Debit Amount' length='25' disabled={isDisabled} required={true} value={object.DebitAmount}/>
                    ...
                    <DataPicker_Day id={'dp_ValueDateDebit_'+suffixID}label='Value Date' disabled={isDisabled}/>
                </Block_Children>
                ...
                <Block_Info>
                    ...
                    <TextField_Value id={'txt_AmountPaid_'+suffixID} label='Amount Credit for Customer' length='25' disabled={true} noDown={true} value={object.PaidAmount}/>
                </Block_Info>
                ...
                <div
                    onClick={() => {
                        if(!isPopup){
                            const currencyID = resolveNameID(currencyList, document.getElementById('slt_CreditCurrency_'+suffixID).innerText)
                            if(currencyID){
                                subCurrency_Data.map(value => {
                                    if(currencyID == value.id){
                                        document.getElementById('txt_CreditAccount_'+suffixID).value = value.Name
                                    }
                                })
                            }
                        }
                    ...
                    <Block_Children header2="CREDIT INFORMATION" >
                        <Select_Object id={'slt_CreditCurrency_'+suffixID} label='Credit Currency'required={true} object={tempCurrency_Data} length='25' disabled={isDisabled} dataID={object.CreditCurrency}/>
                        <TextField_Value id={'txt_CreditAccount_'+suffixID} label='Credit Account' length='25' disabled={true} noDown={true} value={object.CreditAccount}/>
                        ...
                        <DataPicker_Day id={'dp_ExposureDate_'+suffixID}label='Exposure Date' disabled={isDisabled} value={object.ExposureDate}/>
                        <DataPicker_Day id={'dp_ValueDateCredit_'+suffixID}label='Value Date' disabled={isDisabled} value={object.ValueDate}/>
                        <Select_Object id={'slt_Waive_'+suffixID} label='Waive Charges' object={Close_Online} length="25" disabled={isDisabled} dataID={object.WaiveCharges} />
                ...
                <Block_Children header2='BENEFICIARY INFORMATION'>
                    ...
                    <TextField_Value id={'txt_BeneficiaryAccount_'+suffixID} label='Account' length='25' disabled={isDisabled} value={object.BeneficiaryAccount}/>
